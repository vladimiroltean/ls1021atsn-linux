...
static void sja1105et_ptp_cmd_packing(void *buf, struct sja1105_ptp_cmd *cmd,
				      enum packing_op op)
{
	const int size = 4;
...
static void sja1105pqrs_ptp_cmd_packing(void *buf, struct sja1105_ptp_cmd *cmd,
					enum packing_op op)
{
	const int size = 4;
	/* No need to keep this as part of the structure */
...
{
	struct sja1105_ptp_cmd *cmd = (struct sja1105_ptp_cmd *)data;
	const struct sja1105_private *priv = ctx;
	const struct sja1105_regs *regs = priv->info->regs;
	u8 packed_buf[4] = { 0 };

	sja1105et_ptp_cmd_packing(packed_buf, cmd, PACK);
...
int sja1105pqrs_ptp_cmd(const void *ctx, const void *data)
{
	...
	sja1105pqrs_ptp_cmd_packing(packed_buf, cmd, PACK);
