				      ...
				      u64 ts_partial)
{
	...
	WARN_ON(ts_partial >= partial_tstamp_mask);
	...
	if ((now & partial_tstamp_mask) <= ts_partial)
		now -= (partial_tstamp_mask + 1);

	return (now & ~partial_tstamp_mask) | ts_partial;
}
			   ...
			   struct ptp_system_timestamp *sts)
{
	...
	rc = sja1105_xfer_u64(priv, SPI_READ, regs->ptpclk, &ticks, sts);
	if (rc < 0)
...
/* Caller must hold ptp_data->lock */
u64 __sja1105_ptp_gettimex(struct dsa_switch *ds,
			   struct ptp_system_timestamp *sts)
{
	struct sja1105_private *priv = ds->priv;
	u64 ticks;
				...
				struct ptp_system_timestamp *sts)
{
	...
	*ts = ns_to_timespec64(__sja1105_ptp_gettimex(priv->ds, sts));
				   ...
				   struct ptp_system_timestamp *ptp_sts)
{
	...
	return sja1105_xfer_u64(priv, SPI_WRITE, regs->ptpclk, &ticks,
				ptp_sts);
}
