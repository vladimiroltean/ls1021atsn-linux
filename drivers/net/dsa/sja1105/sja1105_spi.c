			    ...
			    u64 reg_addr, void *packed_buf, size_t size_bytes)
{
	...
	if (read_or_write == SPI_READ)
		...
		memcpy(tx_buf + SIZE_SPI_MSG_HEADER, packed_buf,
						size_bytes);
	else
		return -EINVAL;
 ...
 */
int
sja1105_spi_send_int(const struct sja1105_private *priv,
		     enum sja1105_spi_access_mode read_or_write,
				 ...
				 u64 base_addr, void *packed_buf, u64 buf_len)
{
	...
	while (chunk.len) {
		rc = sja1105_spi_send_packed_buf(priv, read_or_write,
				chunk.spi_address, chunk.buf_ptr, chunk.len);
		if (rc < 0)
...
int sja1105_device_id_get(struct sja1105_private *priv)
{
	...
	if (IS_PQRS(priv->device_id)) {
		...
		rc = sja1105_spi_send_int(priv, SPI_READ, ACU_ADDR +
				PROD_ID_ADDR, &tmp_part_nr, 4);
		if (rc < 0)
			goto out;
...
static_config_buf_prepare_for_upload(struct sja1105_private *priv,
				     void *config_buf, int config_buf_len)
{
...
#define RETRIES 10
	...
	config_buf = kcalloc(config_buf_len, sizeof(char), GFP_KERNEL);
	if (!config_buf) {
		rc = -ENOMEM;
		goto out;
	}

	rc = static_config_buf_prepare_for_upload(priv, config_buf,
						  config_buf_len);
	if (rc < 0) {
	...
	do {
		...
		rc = sja1105_spi_send_long_packed_buf(priv, SPI_WRITE,
				CONFIG_ADDR, config_buf, config_buf_len);
		if (rc < 0) {
