...
static int sja1105_apply_mac_settings(struct sja1105_private *priv)
{
	struct sja1105_mac_config_entry default_mac = {
		...
		.enabled = {0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1},
...
static int sja1105_apply_mii_settings(struct sja1105_private *priv,
				      struct sja1105_port *ports)
{
...
static int sja1105_apply_static_fdb(struct sja1105_private *priv)
{
	struct sja1105_table *table;
...
static int sja1105_apply_l2_lookup_params(struct sja1105_private *priv)
{
	struct sja1105_table *table;
	struct sja1105_l2_lookup_params_entry default_l2_lookup_params = {
		/* Learned FDB entries are never forgotten */
		.maxage = 0,
		/* All entries within a FDB bin are available for learning */
		.dyn_tbsz = 4,
		/* 2^8 + 2^5 + 2^3 + 2^2 + 2^1 + 1 in Koopman notation */
		.poly = 0x97,
		/* Include VLAN ID in the FDB entry key */
		.shared_learn = 0,
		.no_enf_hostprt = 0,
...
static int sja1105_apply_static_vlan(struct sja1105_private *priv)
{
	struct sja1105_table *table;
...
static int sja1105_apply_l2_forwarding(struct sja1105_private *priv)
{
...
static int sja1105_apply_l2_forwarding_params(struct sja1105_private *priv)
{
...
static int sja1105_apply_general_params(struct sja1105_private *priv)
{
	...
	struct sja1105_general_params_entry *general;
	struct sja1105_table *table;
...
static int sja1105_apply_l2_policing(struct sja1105_private *priv)
{
	...
	for (i = 0, k = 0; i < SJA1105_NUM_PORTS; i++) {
		...
		for (j = 0; j < SJA1105_NUM_TC; j++, k++) {
			...
			policing[k].smax = 0xffff;
			policing[k].rate = 0xfa00;
			policing[k].maxlen = 1518; /* TODO MTU */
			policing[k].partition = 0;
		}
		/* Set up this port's policer for broadcast traffic */
		policing[bcast].sharindx = bcast;
		policing[bcast].smax = 0xffff;
		policing[bcast].rate = 0xfa00;
		policing[bcast].maxlen = 1518; /* TODO MTU */
				      ...
				      struct sja1105_port *ports)
{
	...
	if ((rc = sja1105_apply_mac_settings(priv)) < 0)
		return rc;
	if ((rc = sja1105_apply_mii_settings(priv, ports)) < 0)
		return rc;
	if ((rc = sja1105_apply_static_fdb(priv)) < 0)
		return rc;
	if ((rc = sja1105_apply_static_vlan(priv)) < 0)
		return rc;
	if ((rc = sja1105_apply_l2_lookup_params(priv)) < 0)
		return rc;
	if ((rc = sja1105_apply_l2_forwarding(priv)) < 0)
		return rc;
	if ((rc = sja1105_apply_l2_forwarding_params(priv)) < 0)
		return rc;
	if ((rc = sja1105_apply_l2_policing(priv)) < 0)
		return rc;
	if ((rc = sja1105_apply_general_params(priv)) < 0)
		return rc;
...
static int sja1105_parse_ports_node(struct sja1105_port *ports,
...
/*
 * Set link speed and enable/disable traffic I/O in the sja1105's mac configuration
 * for a specific port.
 * speed_mbps=0 leaves the speed config unchanged
 * enabled manages rx and tx settings for this port
 */
...
{
	return DSA_TAG_PROTO_PSEUDO_8021Q;
}

static int sja1105_set_drop_policy(struct sja1105_private *priv, int port,
...
static int sja1105_setup_tag_protocol(struct dsa_switch *ds, int port, int enabled)
{
	struct sja1105_private *priv = ds->priv;
	u16 pvid = dsa_get_pseudo_pvid(ds, port);
	int upstream = dsa_upstream_port(ds, port);
	int rc;

	/* CPU port is implicitly configured by configuring the user ports:
	 * we need to add their pvid to its VLAN port membership.
	 * The only specific thing to do for CPU port is to disallow untagged
	 * input if the switch tagging is enabled. TODO: Shouldn't reconfigure
	 * CPU port each time a user port is set up.
	 */
	rc = sja1105_set_drop_policy(priv, upstream, enabled, false);
	if (rc < 0)
		return rc;
	/* Add the user port's pvid to the CPU port's trunk (tagged egress) */
...
static int sja1105_vlan_filtering(struct dsa_switch *ds, int port, bool enabled)
{
	...
	/* VLAN filtering means we can't use pseudo-VLAN tag
	 * for switch tagging. So disable it if filtering is enabled,
	 * and vice-versa.
	 */
	rc = sja1105_setup_tag_protocol(priv->ds, port, !enabled);
	if (rc < 0) {
		dev_err(priv->ds->dev, "Failed to set up tagging protocol\n");
		return rc;
	}
			       ...
			       struct phy_device *phy)
{
	...
	/* By default switch ports don't do forwarding. Therefore it is safe
	 * to assume no VLAN filtering, so we can enable pseudo-VLAN tag now.
	 */
	rc = sja1105_setup_tag_protocol(priv->ds, port, true);
	if (rc < 0) {
		dev_err(priv->ds->dev, "Failed to set up tagging protocol: %d\n", rc);
		return rc;
	}
	return 0;
}
