...
static int sja1105_mgmt_xmit(struct dsa_switch *ds, int port,
			     struct sk_buff *skb)
{
	...
	mgmt_route.tsreg = 0;
	mgmt_route.takets = sp->hwts_tx_en;
	slot = sp->mgmt_slot;
					      ...
					      struct sk_buff *skb)
{
	...
	do {
		rc = sja1105_ptpegr_ts_poll(priv, port, 0, orig_time, &ts);
		if (rc < 0 && rc != -EAGAIN) {
	...
	shwt.hwtstamp = ns_to_ktime(ts.tv_sec * NSEC_PER_SEC +
				    ts.tv_nsec);
	dev_err(priv->ds->dev, "txtstamp 0x%llx\n", shwt.hwtstamp);
...
static void sja1105_port_rxtstamp_work(struct work_struct *work)
{
	...
	while ((skb = skb_dequeue(&sp->skb_rxtstamp_queue)) != NULL) {
		...
		if (!timeout) {
			dev_err(ds->dev, "timed out waiting for meta frame\n");
			kfree_skb(skb);
			continue;
		}

		rc = sja1105_ptp_tstamp_reconstruct(priv,
						    cb->meta_tstamp,
						    cb->orig_time, &ts);
		if (rc < 0)
			continue;
		...
		*shwt = (struct skb_shared_hwtstamps) {0};
		shwt->hwtstamp = ns_to_ktime(ts.tv_sec * NSEC_PER_SEC +
					     ts.tv_nsec);
		dev_err(ds->dev, "rxtstamp 0x%llx\n", shwt->hwtstamp);
		netif_rx_ni(skb);
	}
}
