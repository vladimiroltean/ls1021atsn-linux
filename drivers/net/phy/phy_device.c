...
static void phy_sysfs_create_links(const struct phy_device *phydev)
{
		      ...
		      u32 flags, phy_interface_t interface)
{
	...
	if (ndev_owner != bus->owner && !try_module_get(bus->owner)) {
		phy_err(phydev, "failed to get the bus module\n");
		return -EIO;
	}
	...
	if (!try_module_get(d->driver->owner)) {
		phy_err(phydev, "failed to get the device driver module\n");
		err = -EIO;
